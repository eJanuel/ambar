...
export const SkyRender: React.FC = () => {
  const { gameClock } = useSelector((state: RootState) => state.clock);
  ...
  return (
    <>
      ...
      <Sky distance={distance * 8} sunPosition={sunVector} turbidity={45} rayleigh={0} mieCoefficient={0.005} />
      <Stars
        radius={distance}
        depth={50}
        count={5000}
...
const Sun: React.FC<{ position: { x: number; y: number } }> = ({
  position,
}) => {
  const intensity = position.y > 0 ? 1000000000 : 5;
  return (
    <>
      <Sphere position={[position.x, position.y, 0]} args={[512]}>
...
const Moon: React.FC<{ position: { x: number; y: number } }> = ({
  position,
}) => {
  const intensity = position.y > 0 ? 1000000000 : 1;
  return (
    <>
      <Sphere position={[position.x, position.y, 0]} args={[256]}>
        <meshStandardMaterial
          color={"lightgray"}
          emissive="lightgray"
          emissiveIntensity={intensity}
